...
** Mac上使用Docker
方案有两种：
+ 使用Virtualbox + Boot2docker + 加宿主机器上（也就是Mac上）安装Docker。
+ 使用Virtualbox + Vagrant + CoreOS, 然后在CoreOS上使用Docker。

个人倾向使用后者，因为CoreOS上有许多Docker的工具，而且该系统本身就是为了部署\\
虚拟服务而使用的，而且不需要在Mac上安装Docker。\\
CoreOS介绍请查看: https:////coreos.com/

*** 安装
1. 安装VirturalBox，下载地址https:////www.virtualbox.org/wiki/Downloads
2. 安装Vagrant，下载地址http:////www.vagrantup.com/downloads.html
3. 安装CoreOS
#+BEGIN_SRC sh
...
** 进阶用法（后续补充）
1. 定制Docker image， 使用Dockerfile
2. 使用fleet 管理Docker service
